...
class Wan4DModelCheckpoint(ModelCheckpoint):
    def _remove_checkpoint(self, trainer, filepath):
        ...
        if not match:
            return
        step = match.group(1)
        model_ckpt_path = os.path.join(os.path.dirname(filepath), f"step{step}_model.ckpt")
...
def run_root_from_names(output_base, project_name, experiment_name):
    ...
    return os.path.join(base, project_name.strip(), experiment_name.strip())
...
class Wan4DTrainModule(pl.LightningModule):
    ...
    def _batch_to_model_device(self, batch):
        ...
        temporal_coords = batch["temporal_coords"].to(self.device, dtype=dt)
        temporal_coords = temporal_coords.repeat_interleave(num_views, dim=0)

        plucker_embedding = batch["plucker_embedding"].to(self.device, dtype=dt).flatten(0, 1)
        ...
        with torch.no_grad():
            ...
            pixel_mask = (condition_video.abs().sum(dim=1) > 0).float()
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.log("checkpoint_step", float(self.global_step), on_step=True, logger=False, sync_dist=True)
        return loss
    ...
    def configure_optimizers(self):
        ...
        betas = (float(self.hparams.betas[0]), float(self.hparams.betas[1]))
        ...
        if mode == "constant":
            ...
            lin = torch.optim.lr_scheduler.LinearLR(opt, start_factor=1e-2, total_iters=max(warm, 1))
            cos = torch.optim.lr_scheduler.CosineAnnealingLR(opt, T_max=max(1, total - warm), eta_min=0.0)
            sch = torch.optim.lr_scheduler.SequentialLR(opt, [lin, cos], milestones=[warm]) if warm > 0 else cos
        elif mode == "cosine_warmup":
            lin = torch.optim.lr_scheduler.LinearLR(opt, start_factor=1e-3, end_factor=1.0, total_iters=max(warm, 1))
            cos = torch.optim.lr_scheduler.CosineAnnealingLR(opt, T_max=max(1, total - warm), eta_min=1e-7)
            sch = torch.optim.lr_scheduler.SequentialLR(opt, [lin, cos], milestones=[warm]) if warm > 0 else cos
        else:
            return opt
...
def parse_args():
    p = argparse.ArgumentParser(description="Wan4D train (index.json + Lightning)")
    ...
    p.add_argument("--project_name", type=str, default="wan4d")
    ...
    p.add_argument("--lr_scheduler", type=str, default=WAN21_T2V_13B_LR_SCHEDULER,
    ...
    p.add_argument("--experiment_name", type=str, default="run")
    return p.parse_args()
...
def main():
    ...
    if _is_rank_zero():
        log.info(
            ...
            dataset_root, index_path, len(ds.clips), ds.steps_per_epoch, args.split, args.num_views,
        )
    ...
    cb = [ckpt, LearningRateMonitor(logging_interval="step")]
    trainer = pl.Trainer(
    ...
    if _is_rank_zero():
        log.info("fit resume_ckpt=%s", args.resume_ckpt)
    trainer.fit(model, dl, ckpt_path=args.resume_ckpt)
